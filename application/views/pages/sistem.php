<main>
  <section class="sistem-komposisi-kerja">
    <p class="sistem-top-text" data-aos="fade-up">Sistem & Komposisi Tenaga Kerja</p>
    <h1 class="mt-5 mb-5" data-aos="fade-up">Baladhika Majapahit Menerapkan Program Pengelolaan tenaga Kerja Sebagai Berikut:</h1>
    <div class="program-grid">
      <div class="program-list">
        <h2 data-aos="fade-up" data-aos-delay="100">Bagian Yang Dikelola</h2>
        <ul data-aos="fade-up" data-aos-delay="50">
          <li>Operator Produksi</li>
          <li>Operator Bubut (Finishing/kombinasi)</li>
          <li>Operator Perakitan/Testing</li>
          <li>Bagian Gudang</li>
          <li>Bagian Kebersihan & Taman</li>
          <li>Security</li>
        </ul>
      </div>

      <div class="program-list">
        <h2 data-aos="fade-up" data-aos-delay="100">Jam Kerja</h2>
        <ul data-aos="fade-up" data-aos-delay="50">
          <li>Jam kerja pershift adalah 8 jam setiap harinya, termasuk didalamnya ada istirahat</li>
          <li>Setiap shift bagi tenaga kerja Penunjang Produksi termasuk didalamnya ka shift/ka regu</li>
        </ul>
      </div>

      <div class="program-list">
        <h2 data-aos="fade-up" data-aos-delay="100">Jumlah Tenaga Kerja</h2>
        <P data-aos="fade-up" data-aos-delay="50">
          Jumlah Tenaga Kerja produksi dan penunjang yang dikelola sesuai dengan kebutuhan Perusahaan atau sesuai dengan ketercukupan jumlah personil per-periodenya
        </P>
      </div>

      <div class="program-list">
        <h2 data-aos="fade-up" data-aos-delay="100">Hubungan Kerja</h2>
        <p data-aos="fade-up" data-aos-delay="50">
          Sistem hubungan kerja dengan waktu tertentu
        </p>
      </div>
    </div>
  </section>

  <section class="managerial-system">
    <h1 data-aos="fade-up">Managerial System & Alur Kegiatan</h1>
    <div class="alur-grid">
      <div class="alur-column-left">
        <div class="list-kegiatan" data-aos="fade-up" data-aos-delay="100">
          <p class="warna1">Kontrak Kerjasama</p>
          <div class="warna1">1</div>
        </div>

        <div class="list-kegiatan" data-aos="fade-up" data-aos-delay="100">
          <p class="warna2">Kebutuhan SDM</p>
          <div class="warna2">2</div>
        </div>

        <div class="list-kegiatan" data-aos="fade-up" data-aos-delay="100">
          <p class="warna3">Rekrutmen dan Seleksi</p>
          <div class="warna3">3</div>
        </div>

        <div class="list-kegiatan" data-aos="fade-up" data-aos-delay="100">
          <p class="warna4">Penempatan dan Pemborongan Kerja</p>
          <div class="warna4">4</div>
        </div>

        <div class="list-kegiatan" data-aos="fade-up" data-aos-delay="100">
          <p class="warna5">Managerial sistem dan Pengelolaan SDM</p>
          <div class="warna5">5</div>
        </div>
      </div>

      <div class="image-column" data-aos="fade-up" data-aos-delay="100">
        <img src="assets/image/logo/logo.webp" alt="alur kegiatan foto">
      </div>

      <div class="alur-column-right">
        <div class="list-kegiatan" data-aos="fade-up" data-aos-delay="100">
          <div class="warna5">6</div>
          <p class="warna5">Wajib lapor Tenaga Kerja di Disnaker</p>
        </div>

        <div class="list-kegiatan" data-aos="fade-up" data-aos-delay="100">
          <div class="warna4">7</div>
          <p class="warna4">Pengelolaan Asuransi Tenaga Kerja</p>
        </div>

        <div class="list-kegiatan" data-aos="fade-up" data-aos-delay="100">
          <div class="warna3">8</div>
          <p class="warna3">Pengelolaan Penggajian dan Lembur</p>
        </div>

        <div class="list-kegiatan" data-aos="fade-up" data-aos-delay="100">
          <div class="warna2">9</div>
          <p class="warna2">Pengelolaan Etos Kerja SDM/ Pendidikan dan Latihan</p>
        </div>

        <div class="list-kegiatan" data-aos="fade-up" data-aos-delay="100">
          <div class="warna1">10</div>
          <p class="warna1">Evaluasi Prestasi Kerja</p>
        </div>
      </div>
    </div>

  </section>
</main>